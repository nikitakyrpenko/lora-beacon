...
- **Anchor power: RX duty cycle + deep sleep.** Anchor currently runs continuous LoRa RX with shallow `WFI` sleep between events. SX1280's RxDutyCycle sniff mode (`SetLongPreamble` + `SetRxDutyCycle`) combined with STM32 Stop mode (`SLEEPDEEP`) gives a large power reduction — SX1280 RX vs Sleep current alone is ~35,000:1 (datasheet §3.5.2, 7.5 mA vs 0.215 µA at 1625 kHz) — as long as 2.4 GHz LoRa traffic density on the configured channel stays under the RX/Sleep-window break-even point; every detected preamble costs a full RX window regardless of origin, since this chip has no sync-word content filtering for LoRa (GFSK/BLE/FLRC only). Still needed: size the RX/Sleep windows against a real wake-latency budget, coordinate beacon-side preamble length to guarantee overlap with the anchor's sniff window, reconcile TIM6's ack/ranging deadline timers with Stop mode (TIM6 is APB-clocked and stops there — either restrict deep sleep to states with no TIM6 deadline armed, or migrate those deadlines to LPTIM, which can clock off LSI), and move the watchdog feed's periodic wake source from SysTick (also stops in Stop mode) to RTC — see the recovery item below.
- **Anchor recovery modes.** `LISTENING` -> deep Sleep; `RECOVER` and any TIM6-armed state (`ACK_REQUESTED`/`ACK_IN_PROGRESS`/`RANGING`) -> plain Sleep (TIM6/SysTick both die in Stop mode). `on_recover()`: rate-limited retry -> `to_radio()` once -> on fail, `NRESET_reset()` + `to_radio()` once more -> on fail, `++recover.count` (monotonic, resets only on success) and wait for next retry. RTC (~10s) wakes `LISTENING` from deep sleep to feed IWDG even with no beacon activity. `watchdog_check()` feeds IWDG while `recover.count < 10`, else stops and lets IWDG force a full reboot -- the only backstop for a genuine STM32-side lockup, since everything else here still needs a working CPU to run. Needed: public `get_recover_fail_count()` on `AnchorBridge`, RTC config in the `.ioc`, conditional `SLEEPDEEP` toggle in the main loop, `SystemClock_Config()` re-run after Stop-mode wake.
