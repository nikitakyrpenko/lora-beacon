...
extern "C" uint16_t SX1280_Listen_For_Ack()
{
  ...
  uint8_t irq_mask[8] = {static_cast<uint8_t>(ack_irq_bits >> 8),
                         ...
                         0x0,
                         0x0};
  // 100ms chip-side timeout -- inside the WAKE_ACK_WINDOW_MS (150ms) software fallback ceiling in main.c.
...
extern "C" uint16_t SX1280_Ranging_Master_Mode(uint32_t target_anchor_address)
{
  ...
  // 0x0000 for now, not yet empirically derived (see PLAN.md's calibration research item); this unblocks the
  // exchange completing at all, distance accuracy still needs real calibration once ranging itself works.
...
// 4-step readback dance per SX1280 datasheet (ranging result is only valid on IRQ_BIT_RANGING_MASTER_RESULT_VALID):
// SetStandby(XOSC) -> enable LoRa memory clock (0x97F bit1) -> select debiased result mux (0x924[5:4]) -> read the
// 3-byte result at 0x961-0x963 -> SetStandby(RC). Returns 0 (and leaves *distance_cm_out untouched) on any SPI step
